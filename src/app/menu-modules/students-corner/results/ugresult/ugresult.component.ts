...
export class UGResultComponent implements OnInit {
  ...
  ug_result_table: boolean = false; //ug result table property
  ...
  UG_Result: Results[] = []; //stores PG result
      ...
      this.result.getSeatNumber(seat).subscribe((data) => {
        if (data.length != 0) {
          if (data[0].course == "UG") {
            ...
            this.result_error_message = "Please Enter a UG Seat Number";
